...
test_that("list.files_and_dirs handles empty directory correctly", {
  path <- tempfile()
  dir.create(path)

  result <- list.files_and_dirs(path, pattern = ".*", all.files = TRUE)
  expect_true(result$empty)
})

test_that("list.files_and_dirs filters out binary files and renv directory", {
  path <- tempfile()
  dir.create(path)
  ...
  result <- list.files_and_dirs(path, pattern = exclude_patterns(), all.files = FALSE)
  expect_false(result$empty)
  expect_equal(basename(result$files), "file1.txt")
})

test_that("list.files_and_dirs returns all files if no match with pattern", {
  path <- tempfile()
  dir.create(path)
...
test_that("list.files_and_dirs includes non-empty directories and excludes empty ones", {
  path <- tempfile()
  dir.create(path)
