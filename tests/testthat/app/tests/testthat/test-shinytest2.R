...
test_that("{shinytest2} recording: render_selected_list renders from treeInput and tree_paths.js", {
  ...
  app$set_inputs(`ghqc_app-milestone_description` = "", wait_ = FALSE)
  app$set_inputs(`ghqc_app-milestone` = "", wait_ = FALSE)
  app$set_inputs(`ghqc_app-tree_list` = character(0), wait_ = FALSE)
  app$set_inputs(`ghqc_app-milestone` = "qc id", wait_ = FALSE)
  app$set_inputs(`ghqc_app-tree_list` = c("📄 testthat.R"))
  app$wait_for_idle()

  vals <- app$get_values()
  expect_true(grepl("testthat.R", vals$output$`ghqc_app-main_panel`$html))
  app$expect_screenshot()
})
