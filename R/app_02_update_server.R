  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      if (length(ref_commits()) == 0) {
        return(updateSelectizeInput(session, "ref_commits", choices = "", # needs "" as NULL doesn't give back placeholder
                                    options = list(
    ...
    observe({
      if (!isTruthy(comp_commits())) {
        return(updateSelectizeInput(session, "comp_commits", choices = "", # needs "" as NULL doesn't give back placeholder
                                    options = list(
